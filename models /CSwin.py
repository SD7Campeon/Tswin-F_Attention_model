        ...
        for pre, blocks in zip([self.merge1, self.merge2, self.merge3],
            ...
            for blk in blocks:
                if self.use_chk:
                    ...
                    x = blk(x)
        # x = self.norm(x)#x:tensor[64,49,512]
        # x = torch.mean(x,dim=1)
        return [x]

    def forward(self, x):
        x = self.forward_features(x)#[64,512]
        x_last = self.norm(x[-1])
        # pl= self.avgpool(x.transpose())
        #SUP
...
def CSWin_64_12211_tiny_224(pretrained=False, **kwargs):
    model = CSWinTransformer(patch_size=4, embed_dim=64, depth=[1, 2, 21, 1],drop_path_rate=0.1,
...
def CSWin_64_24322_small_224(pretrained=False, **kwargs):
    model = CSWinTransformer(patch_size=4, embed_dim=64, depth=[2, 4, 32, 2],drop_path_rate=0.4,
                             split_size=[1, 2, 7, 7], num_heads=[2, 4, 8, 16], mlp_ratio=4., **kwargs)
    model.default_cfg = default_cfgs['cswin_224']
    return model
...
def CSWin_96_24322_base_224(pretrained=False, **kwargs):
    model = CSWinTransformer(patch_size=4, embed_dim=96, depth=[2, 4, 32, 2],drop_path_rate=0.3,
